...
struct EnhancerComponent : Component
{
    EnhancerComponent(AudioProcessorValueTreeState &apvts)
    {
        ...
        lfAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                apvts, "lfEnhance", lfEnhance);
        lfEnhance.setTooltip(
            "A saturating, low-end boost after the amp, cab, & reverb. The "
            "frequency is calibrated depending on the amp's mode.");
        ...
        lfAutoGainAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                apvts, "lfEnhanceAuto", lfAutoGain);
        lfAutoGain.setButtonText("Auto");
        lfAutoGain.setTooltip(
            "Enable automatic gain compensation for the enhancer filter.");

        addAndMakeVisible(lfInvert);
        lfInvAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                apvts, "lfEnhanceInvert", lfInvert);
        lfInvert.setButtonText("Inv");
        lfInvert.setTooltip(
            "Invert the low frequency enhancer. At low levels, this works more "
            "like a resonant high-pass, while at higher levels it adds more of "
            "a bell-shaped boost.");

        hfAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                apvts, "hfEnhance", hfEnhance);
        hfEnhance.setTooltip(
            "A saturating, hi-end boost after the amp, cab, & reverb.");
        ...
        hfAutoGainAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                apvts, "hfEnhanceAuto", hfAutoGain);
        hfAutoGain.setButtonText("Auto");
        hfAutoGain.setTooltip(
            "Enable automatic gain compensation for the enhancer filter.");

        addAndMakeVisible(hfInvert);
        hfInvAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                apvts, "hfEnhanceInvert", hfInvert);
        hfInvert.setButtonText("Inv");
        hfInvert.setTooltip(
            "Invert the high frequency enhancer. At low levels, this works "
            "more like a resonant high-pass, while at higher levels it adds a "
            "differently-voiced boost to the high-end.");
    ...
    void paint(Graphics &g) override
    {
        ...
        if (minimized)
            return;
        auto LFLabel = bounds.withTrimmedRight(getWidth() * 0.8f)
                           .withTrimmedBottom(getHeight() * 0.5f)
                           .translated(0, 5)
                           .toNearestInt();
        g.drawFittedText("Low Freq", LFLabel, Justification::centredTop, 1);
        auto HFLabel = bounds.withTrimmedLeft(getWidth() * 0.8f)
                           .withTrimmedBottom(getHeight() * 0.5f)
                           .translated(0, 5)
                           .toNearestInt();
    ...
    void resized() override
    {
        ...
        auto titleBounds = minimized ? bounds.reduced(width * 0.33f, 0)
                                     : bounds.removeFromTop(height * 0.33f)
                                           .reduced(width * 0.33f, 0);
        ...
        auto leftButtons =
            left.removeFromLeft(buttonWidth).reduced(0, height * 0.17f);
        auto rightButtons =
            right.removeFromRight(buttonWidth).reduced(0, height * 0.17f);

        lfCut.setBounds(bounds.removeFromLeft(width * 0.5f).reduced(0, 5));
        hfCut.setBounds(bounds.removeFromLeft(width * 0.5f).reduced(0, 5));

        lfInvert.setBounds(
            leftButtons.removeFromTop(height * 0.5f)
                .withSizeKeepingCentre(buttonWidth, buttonHeight));
        lfAutoGain.setBounds(
            leftButtons.withSizeKeepingCentre(buttonWidth, buttonHeight));
        ...
        hfInvert.setBounds(
            rightButtons.removeFromTop(height * 0.5f)
                .withSizeKeepingCentre(buttonWidth, buttonHeight));
        hfAutoGain.setBounds(
            rightButtons.withSizeKeepingCentre(buttonWidth, buttonHeight));
