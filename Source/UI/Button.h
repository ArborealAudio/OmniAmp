...
struct ButtonLookAndFeel : LookAndFeel_V4
{
    ...
    void drawButtonBackground(Graphics &g, Button &button, const Colour &, bool,
                              bool) override
    {
    ...
    void drawButtonText(Graphics &g, TextButton &button, bool, bool) override
    {
        ...
        g.drawText(text, button.getLocalBounds(), Justification::centred,
                   false);
    }

    float cornerRadius = 0.f;
    bool shouldFillOnToggle = true; /*disable this to avoid filling the button
    with white when it is on, if the button is intended to be a two-way switch
...
struct LightButton : TextButton
{
    ...
    ~LightButton() { setLookAndFeel(nullptr); }
};
...
struct PowerButton : TextButton
{
    ...
    void paint(Graphics &g) override
    {
        ...
        auto drawPowerIcon = [&](Graphics &gc) {
            ...
            gc.fillRoundedRectangle(b.getCentreX() - 1.5f, padding, 3.f,
                                    b.getCentreY() - padding, 2.f);
        };
        ...
        if (!getToggleState())
            ...
            g.drawImage(blur, getLocalBounds().toFloat(),
    ...
    void setBackgroundColor(Colour newColor) { background = newColor; }
...
{
    LinkButton(AudioProcessorValueTreeState &a)
        : apvts(a),
          DrawableButton("Gain Link", DrawableButton::ButtonStyle::ImageFitted)
    {
        attach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                apvts, "gainLink", *this);
        on = Drawable::createFromImageData(BinaryData::link_svg,
                                           BinaryData::link_svgSize);
        overOn = Drawable::createFromImageData(
            BinaryData::link_over_on_svg, BinaryData::link_over_on_svgSize);
        overOff = Drawable::createFromImageData(
            BinaryData::link_over_off_svg, BinaryData::link_over_off_svgSize);
        off = Drawable::createFromImageData(BinaryData::link_off_svg,
                                            BinaryData::link_off_svgSize);
        setClickingTogglesState(true);
        setTooltip("Link Input and Output gains. Output gain can still be "
                   "adjusted with linking on.");
    }
    ...
    void paint(Graphics &g) override
    {
        if (isMouseOver()) {
            if (getToggleState())
                overOn->drawWithin(g, getLocalBounds().toFloat(),
                                   RectanglePlacement::centred, 1.f);
            else
                overOff->drawWithin(g, getLocalBounds().toFloat(),
                                    RectanglePlacement::centred, 1.f);
        } else if (getToggleState()) {
            on->drawWithin(g, getLocalBounds().toFloat(),
                           RectanglePlacement::centred, 1.f);
        } else {
            off->drawWithin(g, getLocalBounds().toFloat(),
                            RectanglePlacement::centred, 1.f);
        }
    }
...
struct ResizeButton : TextButton
{
    ...
    void paint(Graphics &g) override
    {
        ...
        g.strokePath(p, PathStrokeType(2.f, PathStrokeType::JointStyle::curved,
