...
{
    AudioSource() { startTimerHz(45); }
    ~AudioSource() override { stopTimer(); }
    ...
    template <typename T> void copyBuffer(const AudioBuffer<T> &buffer)
    {
    ...
    inline std::array<float, 6> getAvgRMS()
    {
        ...
        for (size_t i = 0; i < 6; ++i) {
            if (src[i].size() > 0)
                b_rms[i] = std::sqrt(
                    std::accumulate(src[i].begin(), src[i].end(), 0.f) /
                    (float)src[i].size());
            else
    ...
    {
        {
            ...
            for (auto &b : band) {
                if (scope.blockSize1 > 0) {
                    b.copyFrom(0, 0,
                               mainBuf.getReadPointer(0) + scope.startIndex1,
                               scope.blockSize1);
                }
                if (scope.blockSize2 > 0) {
                    b.copyFrom(0, scope.blockSize1,
                               mainBuf.getReadPointer(0) + scope.startIndex2,
                               scope.blockSize2);
                }
            }
        }
...
{
    SineWaveComponent(AudioSource &s) : src(s) { startTimerHz(45); }
    ~SineWaveComponent() override { stopTimer(); }

    void paint(Graphics &g) override
    {
        g.setColour(Colour(BACKGROUND_COLOR).withAlpha(0.9f));
        g.fillRoundedRectangle(getLocalBounds().toFloat(), getHeight() * 0.1f);
        drawSineWave(g);
        g.setColour(Colour(DEEP_BLUE));
        g.drawRoundedRectangle(getLocalBounds().reduced(1).toFloat(),
                               getHeight() * 0.1f, 2.f);
    }
    ...
    inline void drawSineWave(Graphics &g)
    {
        ...
        Path p[6];
        Colour colors[6]{Colour(0xff975755), Colour(0xff977757),
                         Colour(0xff859358), Colour(0xff499481),
                         Colour(0xff516692), Colour(0xff975792)};
        ...
        auto drawWave = [&](size_t i) {
            ...
            FloatVectorOperations::add(
                x.data(), -f, x.size()); /*much faster w/ float vec ops!*/
