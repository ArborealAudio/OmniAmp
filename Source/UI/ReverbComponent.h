...
class ReverbComponent : public Component
{
    ...
    Knob reverbAmount{knobFlags}, reverbDecay{knobFlags}, reverbSize{knobFlags},
        predelay{knobFlags};
    std::unique_ptr<AudioProcessorValueTreeState::SliderAttachment> amtAttach,
        decayAttach, sizeAttach, predelayAttach;
  ...
  public:
    ReverbComponent(AudioProcessorValueTreeState &v)
        : reverb(v.getParameter("reverbType")->getAllValueStrings())
    {
        addAndMakeVisible(reverbAmount);
        amtAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                v, "reverbAmt", reverbAmount);
        reverbAmount.setLabel("Amount");
        reverbAmount.setColor(
            Colours::antiquewhite,
            Colours::antiquewhite.withMultipliedLightness(1.5f));
        reverbAmount.setTooltip(
            "Mix for reverb. At 50%, wet and dry signals are both at full "
            "volume, and the dry signal begins to decrease after 50%");
        ...
        decayAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                v, "reverbDecay", reverbDecay);
        reverbDecay.setLabel("Decay");
        reverbDecay.setColor(
            Colours::antiquewhite,
            Colours::antiquewhite.withMultipliedLightness(1.5f));
        ...
        reverbBright.setTooltip("Switch on Bright mode for the reverb, which "
                                "lets through more high frequencies.");
        brightAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                v, "reverbBright", reverbBright);
    ...
    void paint(Graphics &g) override
    {
        ...
        reverb.lnf.backgroundColor = reverb.getSelectedId() > 1
                                         ? Colour(REVERB_COLOR)
    ...
    void resized() override
    {
        ...
        // reverb.setBounds(b.withSizeKeepingCentre(b.getWidth() * 0.75f,
        // b.getHeight() * 0.66f));
