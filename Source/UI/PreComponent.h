...
struct PreComponent : Component
{
    ...
    PreComponent(strix::VolumeMeterSource &vs, AudioProcessorValueTreeState &v)
        : vts(v), grMeter(vs,
                          strix::VolumeMeterComponent::Horizontal |
                              strix::VolumeMeterComponent::Reduction |
                              strix::VolumeMeterComponent::Background,
                          30, vts.getRawParameterValue("comp"))
    {
        for (auto c : getComps()) {
            addAndMakeVisible(*c);
        ...
        emphasisAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                vts, "stereoEmphasis", emphasis);
        emphasis.setDefaultValue(
            vts.getParameter("stereoEmphasis")->getDefaultValue());
        emphasis.setLabel("Stereo Emphasis");
        emphasis.setTooltip(
            "Stereo emphasis control--a compensated drive control for the "
            "stereo width going *into* the processing.\n\nUse this for "
            "fine-tuning the compression or saturation of the stereo sides.");
        ...
        midSide.setTooltip(
            "Toggle for Stereo and Mid/Side processing.\n\nIn Stereo mode, "
            "Left and Right channels will be processed independently in the "
            "amp. In Mid/Side mode, the middle of the stereo image will be "
            "separated from the sides and then processed "
            "independently.\n\nThis is useful if you want to do Mid/Side "
            "compression or saturation, which can create a more spacious "
            "stereo image.");
        ...
        lfEmph.setTooltip(
            "Adds or subtracts low-end before all saturation and compression, "
            "then an equal and opposite compensation after the saturation and "
            "compression. Useful for getting more saturation from the low-end "
            "without boosting it in volume.");

        lfFreqAttach =
            std::make_unique<AudioProcessorValueTreeState::SliderAttachment>(
                vts, "lfEmphasisFreq", lfFreq);
        ...
        hfEmph.setTooltip(
            "Adds or subtracts high-end before all saturation and compression, "
            "then an equal and opposite compensation after the saturation and "
            "compression. Useful for getting more saturation from the high-end "
            "without boosting it in volume.");
        ...
        comp.setTooltip(
            "An opto-style compressor, with signal-dependent attack and "
            "release. Increasing gives more compression and sustain, and can "
            "also boost the volume going into the amp.");

        compLinkAttach =
            std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(
                vts, "compLink", compLink);
    ...
    {
        minimized = state;
        resizeButton.setToggleState(!state,
    ...
    void paint(Graphics &g) override
    {
        ...
        if (!minimized) {
            float div =
                doubler.getRight() + (grMeter.getX() - doubler.getRight()) / 2;
            g.fillRoundedRectangle(div - 1.5f, 8.f, 3.f, getHeight() - 16.f,
                                   3.f);
        }
    }
    ...
    void resized() override
    {
        ...
        grMeter.setBounds(
            compSection.removeFromBottom(compSection.getHeight() * 0.33f)
                .reduced(compSectW * 0.03f));
        const auto compSectH =
            compSection.getHeight(); // compSect height after GR meter
        auto w = bounds.getWidth();
        int chunk = w / 5;

        auto msBounds =
            minimized ? Rectangle(0, 0, 0, 0)
                      : bounds.removeFromLeft(chunk).withSizeKeepingCentre(
                            chunk * 0.66f, bounds.getHeight() * 0.33f);
        auto titleBounds =
            minimized ? bounds.removeFromLeft(chunk).withSizeKeepingCentre(
                            chunk * 0.66f, bounds.getHeight())
        ...
        title.setFont(
            Font(title.getHeight() * 0.65f).withExtraKerningFactor(0.25f));

        resizeButton.setBounds(Rectangle(title.getRight() - 10, title.getY(),
                                         title.getHeight(), title.getHeight())
                                   .reduced(12));
        ...
        for (auto *c :
             getSelectComps(StereoEmphasis | LFEmph | HFEmph | Doubler)) {
            if (c == &lfEmph) {
                auto subBounds = bounds.removeFromLeft(chunk);
                layoutComponents(getSelectComps(LFEmph | LFFreq), subBounds,
                                 true, 0.7f, 0.025f);
                lfFreq.cornerRadius = lfFreq.getHeight() * 0.5f;
            } else if (c == &hfEmph) {
                auto subBounds = bounds.removeFromLeft(chunk);
                layoutComponents(getSelectComps(HFEmph | HFFreq), subBounds,
                                 true, 0.7f, 0.025f);
        ...
        auto compKnobBounds =
            compSection.removeFromLeft(compSectW * 0.5f).reduced(5, 0);
        comp.setBounds(compKnobBounds);
        comp.setOffset(0, -5);
        comp.setTextOffset(0, -2);
        compPos.setBounds(compSection.removeFromTop(compSectH * 0.5f - 2)
                              .reduced(compSectW * 0.125f, compSectH * 0.07f));
        compLink.setBounds(compSection.removeFromTop(compSectH * 0.5f - 2)
                               .reduced(compSectW * 0.125f, compSectH * 0.07f));
  ...
  private:
    ...
    Knob::Flags knobFlags =
        Knob::DRAW_GRADIENT | Knob::DRAW_SHADOW | Knob::DRAW_ARC;

    Knob emphasis{knobFlags}, doubler{knobFlags}, lfEmph{knobFlags},
        hfEmph{knobFlags}, comp{knobFlags};
    ...
    std::unique_ptr<AudioProcessorValueTreeState::SliderAttachment>
        emphasisAttach, doublerAttach, lfAttach, lfFreqAttach, hfAttach,
        hfFreqAttach, compAttach;
    std::unique_ptr<AudioProcessorValueTreeState::ButtonAttachment> msAttach,
        compLinkAttach, compPosAttach;
    ...
    {
        return {&midSide, &emphasis, &lfEmph, &lfFreq,  &hfEmph,
                &hfFreq,  &doubler,  &comp,   &compPos, &compLink};
    }
