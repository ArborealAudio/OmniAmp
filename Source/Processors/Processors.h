...
/**
 * A struct which contains a vector of processors and calls their respective
 * process methods
 */
struct Preamp
{
...
{
    Processor(AudioProcessorValueTreeState &a, ProcessorType t,
              strix::VolumeMeterSource &s)
        : apvts(a), comp(t, s, a.getRawParameterValue("compPos"))
    {
        inGain = dynamic_cast<strix::FloatParameter *>(
            apvts.getParameter("preampGain"));
        lastInGain = *inGain;
        ampAutoGain = dynamic_cast<strix::BoolParameter *>(
            apvts.getParameter("ampAutoGain"));
        outGain = dynamic_cast<strix::FloatParameter *>(
            apvts.getParameter("powerampGain"));
        lastOutGain = *outGain;
        p_comp =
            dynamic_cast<strix::FloatParameter *>(apvts.getParameter("comp"));
        linked = dynamic_cast<strix::BoolParameter *>(
            apvts.getParameter("compLink"));
        hiGain =
            dynamic_cast<strix::BoolParameter *>(apvts.getParameter("hiGain"));
        dist =
            dynamic_cast<strix::FloatParameter *>(apvts.getParameter("dist"));
    ...
    virtual void setDistParam(float newValue) { mxr.setParams(1.f - newValue); }
    ...
    virtual strix::VolumeMeterSource &getActiveGRSource()
    {
        return comp.getGRSource();
    }
  ...
  protected:
    ...
    double fudgeGain =
        1.f; // dumb thing we need to keep diff. amp types in similar ballpark
...
template <typename T> struct Guitar : Processor
{
    Guitar(AudioProcessorValueTreeState &a, strix::VolumeMeterSource &s)
        : Processor(a, ProcessorType::Guitar, s)
    {
        guitarMode = dynamic_cast<strix::ChoiceParameter *>(
            apvts.getParameter("guitarMode"));
        currentType = static_cast<GuitarMode>(guitarMode->getIndex());
#if USE_SIMD
        toneStack =
            std::make_unique<ToneStack<vec>>(ToneStack<vec>::Type::Nodal);
#else
        toneStack =
            std::make_unique<ToneStack<double>>(ToneStack<double>::Type::Nodal);
#endif
    ...
    ~Guitar() { apvts.removeParameterListener("guitarMode", this); }
    ...
    void setToneStack()
    {
        switch (currentType) {
        case GammaRay:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)25e-9, (T)22e-9, (T)300e3,
                                      (T)1e6, (T)20e3, (T)65e3);
            break;
        case Sunbeam:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)15e-9, (T)250e-9, (T)300e3,
                                      (T)400e3, (T)1e3, (T)20e3);
            break;
        case Moonbeam:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)20e-9, (T)50e-9, (T)300e3,
                                      (T)500e3, (T)5e3, (T)12e3);
            break;
        case XRay:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)22e-9, (T)20e-9, (T)270e3,
                                      (T)1e6, (T)50e3, (T)65e3);
            break;
        }
    }
    ...
    void processBlock(dsp::AudioBlock<FloatType> &block)
    {
        ...
        strix::SmoothGain<T>::applySmoothGain(processBlock, gain_raw,
                                              lastInGain);
        // if (ampAutoGain_)
        ...
        strix::SmoothGain<T>::applySmoothGain(processBlock, out_raw,
                                              lastOutGain);
...
template <typename T> struct Bass : Processor
{
    Bass(AudioProcessorValueTreeState &a, strix::VolumeMeterSource &s)
        : Processor(a, ProcessorType::Bass, s)
    {
        bassMode = dynamic_cast<strix::ChoiceParameter *>(
            apvts.getParameter("bassMode"));
    ...
    ~Bass() { apvts.removeParameterListener("bassMode", this); }
    ...
    void setToneStack()
    {
        switch (currentType) {
        case Cobalt:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)25e-9, (T)15e-9, (T)250e3,
                                      (T)500e3, (T)50e3, (T)200e3);
            break;
        case Emerald:
            toneStack->setBiquadFreqs(300.0, 900.0, 200.0, 1000.0, 2300.0);
            break;
        case Quartz:
            toneStack->setNodalCoeffs((T)0.25e-9, (T)8e-9, (T)12e-9, (T)250e3,
                                      (T)750e3, (T)100e3, (T)300e3);
            break;
        }
    }
...
template <typename T> struct Channel : Processor
{
    Channel(AudioProcessorValueTreeState &a, strix::VolumeMeterSource &s)
        : Processor(a, ProcessorType::Channel, s)
    {
        toneStack = nullptr;
        channelMode = dynamic_cast<strix::ChoiceParameter *>(
            apvts.getParameter("channelMode"));
    ...
    ~Channel() { apvts.removeParameterListener("channelMode", this); }
    ...
    void update(const dsp::ProcessSpec &spec, const float lowGain = 0.5f,
                const float midGain = 0.5f, const float trebleGain = 0.5f)
    {
        SR = spec.sampleRate;
    ...
    void setFilters(int index, float newValue = 0.5f)
    {
        ...
        case 1: {
            ...
            case Vintage:
                *mid.coefficients =
                    dsp::IIR::ArrayCoefficients<double>::makePeakFilter(
                        SR, 800.0, Q * 0.75, gain);
                break;
            case Biamp:
                if (gain >= 1.0)
                    *mid.coefficients =
                        dsp::IIR::ArrayCoefficients<double>::makePeakFilter(
                            SR, 2900.0, M_SQRT1_2 * 0.6, gain);
                else
                    *mid.coefficients =
                        dsp::IIR::ArrayCoefficients<double>::makePeakFilter(
                            SR, 2900.0, M_SQRT1_2 * 0.4, gain);
                break;
            }
        } break;
        case 2:
            ...
            case Biamp:
                if (gain >= 1.0) {
                    ...
                    *hi.coefficients =
                        dsp::IIR::ArrayCoefficients<double>::makePeakFilter(
                            SR, freq, M_SQRT1_2 * q, gain);
                } else
                    *hi.coefficients =
                        dsp::IIR::ArrayCoefficients<double>::makePeakFilter(
                            SR, 10e3,
                            M_SQRT1_2 * jmap(gaindB / minDb, 0.154f, 0.522f),
                            gain);
                break;
            }
            break;
    ...
    void processBlock(dsp::AudioBlock<FloatType> &block)
    {
        ...
        case ProcessRampOn: {
            for (size_t ch = 0; ch < processBlock.getNumChannels(); ++ch)
                tmp.copyFrom(ch, 0, processBlock.getChannelPointer(ch),
                             processBlock.getNumSamples());
#if USE_SIMD
            ...
            if (*hiGain)
                triode[1].process(processBlock);
            strix::Crossfade::process(tmpBlock, processBlock,
                                      processBlock.getNumSamples());
        } break;
        case ProcessRampOff: {
            for (size_t ch = 0; ch < processBlock.getNumChannels(); ++ch)
        ...
        if (ampAutoGain_)
            autoGain *=
                1.0 / std::sqrt(std::sqrt(gain_raw * gain_raw * gain_raw));
        ...
        strix::SmoothGain<T>::applySmoothGain(processBlock, autoGain,
                                              lastAutoGain);
#if USE_SIMD
        simd.deinterleaveBlock(processBlock);
#endif
  ...
  private:
    ...
    template <class Block> void processFilters(Block &block)
    {
