...
struct GuitarPostFilters
{
    ...
    void prepare(const dsp::ProcessSpec& spec)
    {
        ...
        *sc_hp.coefficients = dsp::IIR::ArrayCoefficients<float>::makeHighPass(SR, 70.f);
        ...
        *sc_lp.coefficients = dsp::IIR::ArrayCoefficients<float>::makeLowPass(SR, 8000.f);
    }

    void updateLowPass(bool hi)
    {
        if (hi)
            *sc_lp.coefficients = dsp::IIR::ArrayCoefficients<float>::makeLowPass(SR, 10000.f);
        else
            *sc_lp.coefficients = dsp::IIR::ArrayCoefficients<float>::makeLowPass(SR, 8000.f);
    }
...
private:
    ...
    dsp::IIR::Filter<float> sc_hp, peak1, peak2, peak3, peak4, sc_lp;
